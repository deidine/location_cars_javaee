					...
					<div class="card mb-4 box-shadow pt-3 pb-5">
						<div class="card-body"
							...
							<h5 class="card-text pb-3">
								...
								<%=voiture.getVille()%></h5>
								</div><a href="${pageContext.request.contextPath}/ChatServlet?receiver=<%=voiture.getId_user()%>"> <button class="btn btn-primary" style='font-size:24px'>chat me <i class='far fa-comment-dots'></i></button></a>
					<form method="post" action="${pageContext.request.contextPath}/ChatServlet">
								<input type="text" value="<%=voiture.getId_user()%>" name="receiver"> 
								<button type="submit" class="btn btn-primary" style='font-size:24px'>chat me <i class='far fa-comment-dots'></i></button>
								</form> 
					</div>
				</div>
			</div>
